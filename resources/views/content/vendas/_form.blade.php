<div class="row">
    ...
    <div class="col-md-6 mb-3">
        ...
        <input type="datetime-local" class="form-control" id="data_venda" name="data_venda" value="{{ date('Y-m-d\TH:i') }}" required>
    </div>
    ...
    <div class="col-md-5 mb-3">
        ...
        <select class="form-select select2" id="produto_selector">
            ...
            @foreach($produtos as $produto)
                <option value="{{ $produto->id_produto }}" data-price="{{ $produto->valor_venda }}" data-image="{{ $produto->imagem ? asset('storage/' . $produto->imagem) : '' }}">{{ $produto->nome }} (R$ {{ number_format($produto->valor_venda, 2, ',', '.') }})</option>
            @endforeach
        </select>
    </div>
    ...
    <div class="col-md-2 mb-3 d-flex align-items-end">
        <button type="button" class="btn btn-primary w-100" id="btn_add_item"><i class="ri-add-line me-1"></i> Adicionar</button>
    </div>
                ...
                <div class="col-md-4 mb-2">
                    ...
                    <select class="form-select" id="ing_produto_selector">
                        ...
                        @foreach($produtos as $produto)
                            ...
                            <option value="{{ $produto->id_produto }}" data-nome="{{ $produto->nome }}" data-batches='{{ json_encode($produto->estoque) }}'>{{ $produto->nome }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4 mb-2">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    btnAdd.addEventListener('click', function() {
        ...
        if (isManipulated) {
            ...
            if(currentIngredients.length === 0) return alert('Adicione pelo menos um ingrediente para a manipulação.');

            manDetails = `<span class="badge bg-warning">Manipulado</span><br><small>Máq: ${maqSel.options[maqSel.selectedIndex].text}<br>Fórm: ${formCode}</small>`;
            manInputs = `
        ...
        const row = `
            <tr>
                ...
                <td>
                    ${qty}
                    <input type="hidden" name="itens[${itemCount}][quantidade]" value="${qty}">
                </td>
                <td>
                    R$ ${price.toFixed(2)}
                    <input type="hidden" name="itens[${itemCount}][valor_unitario]" value="${price}">
                </td>
    ...
    function updateTotal() {
        ...
        document.querySelectorAll('.item-total').forEach(td => {
            total += parseFloat(td.innerText.replace('R$ ', ''));
        });
